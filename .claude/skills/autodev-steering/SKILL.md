...
プロジェクトの実態を確認する。本プロジェクト（gemini-mcp）では以下のコマンドを参考にする:

```bash
ls src/                              # ソースツリー確認
cat package.json | jq '.scripts'     # 利用可能スクリプト一覧
cat package.json | jq '.dependencies, .devDependencies'  # 依存状況
cat manifest.json 2>/dev/null        # DXT マニフェスト（存在すれば）
ls .ai-agent/tasks/ .ai-agent/projects/ .ai-agent/surveys/  # 進行中の作業
git log --oneline -20                # 直近の変更
```

確認ポイント:

- `@google/gemini-cli` 以外の Gemini API 系依存が紛れ込んでいないか
- DXT 関連ファイル（`manifest.json`、`pack:dxt` スクリプト）の整備状況
- Gemini CLI のサブプロセス起動を行うコードの所在
