...
実装段階では以下を実行して品質を担保する（コマンドが整備された段階で順次有効化）:
...
DXT パッケージの動作は、以下の手順で手動検証する:

1. `npm run pack:dxt` で `.dxt` を生成
2. Claude Desktop の拡張機能マネージャーから `.dxt` を読み込み
3. `gemini auth login` 済みの状態で、Claude Desktop からツール呼び出しを実行

### プロダクト固有の制約（必読）

- **Gemini API SDK を直接使わない** — 必ず `@google/gemini-cli` 経由とする
- **外部ストレージ（Redis 等）を導入しない** — ローカル MCP サーバーとして自己完結を維持
- **Workspace OAuth 認証を壊さない** — Gemini CLI 側の認証状態に依存する設計を保つ

詳細は `.ai-agent/steering/product.md` の非スコープ項目および `tech.md` の依存ポリシーを参照。
